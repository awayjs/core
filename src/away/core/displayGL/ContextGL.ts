...
module away.displayGL
{
	...
	export class ContextGL
	{
		...
		private _samplerStates:away.displayGL.SamplerState[] = [];
		...
		public createCubeTexture(size:number, format:away.displayGL.ContextGLTextureFormat, optimizeForRenderToTexture:boolean, streamingLevels:number = 0):away.displayGL.CubeTexture
		{
			var texture:away.displayGL.CubeTexture = new away.displayGL.CubeTexture(this._gl, size);
			this._textureList.push(texture);
			return texture;
		}

		public createIndexBuffer(numIndices:number):away.displayGL.IndexBuffer
		{
			var indexBuffer:IndexBuffer = new away.displayGL.IndexBuffer(this._gl, numIndices);
		...
		{
			var program:Program = new away.displayGL.Program(this._gl);
		...
		{
			//TODO streaming
			var texture:Texture = new away.displayGL.Texture(this._gl, width, height);
			this._textureList.push(texture);
			return texture;
		}

		public createVertexBuffer(numVertices:number, data32PerVertex:number):away.displayGL.VertexBuffer
		{
			var vertexBuffer:VertexBuffer = new away.displayGL.VertexBuffer(this._gl, numVertices, data32PerVertex);
		...
		public setProgram(program:away.displayGL.Program)
		{
		...
		public setTextureAt(sampler:number, texture:away.displayGL.TextureBase)
		{
		...
		public setGLSLTextureAt(locationName:string, texture:TextureBase, textureIndex:number)
		{
			...
			if (texture.textureType == "texture2d") {
				this._gl.bindTexture(this._gl.TEXTURE_2D, (<away.displayGL.Texture>texture).glTexture);
				this._gl.uniform1i(location, textureIndex);
				var samplerState:away.displayGL.SamplerState = this._samplerStates[ textureIndex ];
				...
				this._gl.bindTexture(this._gl.TEXTURE_CUBE_MAP, (<away.displayGL.CubeTexture>texture).glTexture);
				this._gl.uniform1i(location, textureIndex);
				var samplerState:away.displayGL.SamplerState = this._samplerStates[ textureIndex ];
		...
		public setRenderToTexture(target:away.displayGL.TextureBase, enableDepthAndStencil:boolean = false, antiAlias:number = 0, surfaceSelector:number = 0)
		{
			var frameBuffer:WebGLFramebuffer = (<away.displayGL.Texture>target).frameBuffer
